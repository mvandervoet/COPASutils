...
if(getRversion() >= "2.15.1")  utils::globalVariables(c(".", "Plate1", "Plate2",
 ...
 "presentation", "bubbleSVMmodel_noProfiler", "vectorFilename", "vectorRaw"))
...
readSorter <- function(file, tofmin=0, tofmax=10000, extmin=0, extmax=10000, reflx=FALSE)  {
...
readPlate <- function(file, tofmin=0, tofmax=10000, extmin=0, extmax=10000, SVM=TRUE, reflx=FALSE) {
...
plotTrait = function(plate, trait, trait2=NULL, type="heat"){
 ...
 if(type == "heat"){
  ...
  plot = ggplot(plate) + geom_rect(aes_string(xmin=0,xmax=5,ymin=0,ymax=5,fill=trait)) +
      ...
      theme(axis.ticks.x=element_blank(), axis.ticks.y=element_blank(), axis.text.x=element_blank(), axis.text.y=element_blank()) + ggtitle(platename) +
      xlab("Columns") + ylab("Rows") + theme(plot.title = element_text(hjust = 0.5))
 } else if(type == "scatter"){
  plot = ggplot(plate) + geom_point(aes_string(x = trait, y = trait2), size=0.5, alpha=1/2) + xlab(trait) + ylab(trait2) + ggtitle(platename) +
      theme(axis.text.x=element_text(size="8", angle=45, hjust=1), axis.text.y=element_text(size="8"), plot.title = element_text(hjust = 0.5))
 } else if(type == "hist"){
  ...
  plot = ggplot(plate) + geom_histogram(aes_string(x = trait), binwidth = diff(range(plate[[trait]]))/15) + xlab(trait) + ylab("Count") + ggtitle(platename) +
      theme(axis.text.x=element_text(size="8", angle=45, hjust=1), axis.text.y=element_text(size="8"), plot.title = element_text(hjust = 0.5))
 } else {
...
plotCorMatrix = function(plate_summary1, plate_summary2=plate_summary1){
 platename1 <- deparse(substitute(plate_summary1))
 platename2 <- deparse(substitute(plate_summary2))
 ...
 ggplot(corDF, aes(Plate1, Plate2, fill = Correlation)) + 
  ...
  theme(axis.text.x=element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) + xlab(platename1) + ylab(platename2)
}
...
plotCompare = function(plates, trait, plateNames=NULL){
 ...
 if(nrow(wholeDF) %% 96 == 0 & trait %in% colnames(wholeDF)){
  ...
  ggplot(wholeDF, aes_string(x = "Plate", y = trait, fill = "Plate")) + geom_boxplot(size=0.25, outlier.size = 0.25, outlier.alpha = 0.25) + facet_grid(row~col, drop=FALSE) + theme(axis.ticks.x = element_blank(), axis.text.x = element_blank())
 }
}
...
plotDRTrait = function(plate_summary, concentrations, trait="n"){
 platename <- deparse(substitute(plate_summary))  
 plate_summary = data.frame(cbind(concentration = concentrations, plate_summary))
 plate_summary = plate_summary[,-c(3,4)]
 plate_summary = plate_summary[!is.na(plate_summary$compound)&!is.na(plate_summary$concentration),]
 plate_summary = melt(plate_summary, id=c("compound", "concentration"))
 plate_summary = plate_summary %>% group_by(compound, concentration, variable) %>% summarize(mean = mean(value))
 plate_summary = reshape2::dcast(plate_summary, compound+concentration~variable, value.var="mean")
 ggplot(plate_summary, aes_string(x="concentration", y=trait, colour="compound")) + 
   ggtitle(platename) + 
   theme(plot.title = element_text(hjust = 0.5)) + 
   geom_point() + 
   geom_line() + 
   scale_colour_discrete(name="Compound", guide=FALSE) + 
   xlab("Concentration") + 
   ylab(trait) + 
...
#Generate a plot for each compound
plotCompound = function(plate_summary){
  platename <- deparse(substitute(plate_summary))
  ggplot (plate_summary, aes(log10(concentration), n)) + geom_point(aes(color = compound), size = 2, alpha = 1/2) + facet_wrap(~compound, scales="free", ncol=2) + theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5)) + stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1) + ylim(-100,(max(plate_summary$n, na.rm=TRUE)+1000)) + ggtitle(platename)
}




# Automate the analysis by generating and saving plots
# Looping structure
# Analyze pipeline
generatePlots <- function(experiment, platename, compounds, concentrations, loops, tofcutoff='800', reflx=FALSE){
  
  #User input
  message("To get started I need some information:")
  loops=1
  loops <- as.numeric(readline("Should I analyze 1 or 2 (duplicate!) plates: "))
  experiment <- readline("Filename of experiment 1 (e.g. C5.txt): ")
  platename <- readline("The name to display on the plots (e.g. C5): ")
  #Set parameters based on the number of plates to analyze (1 or 2)
  if(loops==1){
    toPlotAll <- "Exp1All"
    toPlotSumAdult <- "sumExp1Adult"
    toPlotSumNonAdult <- "sumExp1NonAdult"
    toPlotNonAdult <- "Exp1NonAdult"
    toPlotSumDRNonAdult <- "sumDRExp1NonAdult"
  }
  if(loops==2){
    toPlotAll <- c("Exp1All","Exp2All")
    toPlotSumAdult <- c("sumExp1Adult","sumExp2Adult")
    toPlotSumNonAdult <- c("sumExp1NonAdult","sumExp2NonAdult")
    toPlotNonAdult <- c("Exp1NonAdult","Exp2NonAdult")
    toPlotSumDRNonAdult <- c("sumDRExp1NonAdult","sumDRExp2NonAdult")
    experiment <- c(experiment, readline("Filename of experiment 2 (e.g. D7.txt): "))
    platename <- c(platename, readline("The name to display on the plots (e.g. D7): "))
  }
  compounds <- unlist(strsplit(read_file(readline("Filename of the compounds file (e.g. compounds1.txt): ")), split=","))
  concentrations <- as.numeric(unlist(strsplit(read_file(readline("Filename of the concentrations file (e.g. concentrations1.txt): ")), split=",")))
  tofcutoff <- as.numeric(readline("Cut-off TOF value to seperate P0 and F1 (e.g. 800, L4 = 650, young adult = 900): "))
  
  message("\nGreat! I will generate the plots, this will take a few minutes.\nI'll keep you updated on the progress...\n")
  
  #Functions for tidying data: tidyTail and tidySource
  tidyTail <- function(filename, platename) {
    #remove tail, based on empty line or Instrument
    for (value in filename)
    {
      #Define the file name that will be deleted
      fnstep1 <- paste("TidyDataStep1_",platename,".csv",sep="")
      fnstep2 <- paste("TidyDataStep2_",platename,".csv",sep="")
      #Check its existence
      if (file.exists(fnstep1)){
        #Delete file if it exist
        file.remove(fnstep1)
      }
      if (file.exists(fnstep2)){
        #Delete file if it exist
        file.remove(fnstep2)
      }
      message(paste("Tidying Files Step 1 ",platename,"...",sep=""))
      con = file(experiment[count], "r")
      message("working...")
      while ( TRUE ) {
        line = readLines(con, n = 1)
        if ( line == "" | grepl("INSTRUMENT", line) ) {
          
          break
        }
        write(line,paste("TidyDataStep1_",platename,".csv",sep=""),append=TRUE)
      }
      close(con)
    }
  }
    
  tidySource <- function(experiment, platename) {
    #split Source column into Row and Column column
    count = 1
    for (value in experiment) {
      message(paste("Tidying Files Step 2 ",platename[count],"...",sep=""))
      df <- read.csv(experiment[count], header=TRUE, sep ="\t")
      if("Source.well" %in% colnames(df))
      {
        df2 <- transform(df, Row = substr(Source.well, 1, 1), Column = substr(Source.well, 2, 3))
        write.table(df2, file = paste("TidyDataStep2_",platename[count],".csv",sep=""), quote=FALSE, sep="\t", row.names = FALSE)
      }
      count=count+1
      message("working...")
    }
  ...
  #Tidy data
  count=1
  vectorFilename <<- vector(mode="character", length=0)
  #vectorFilename<-vector()
  for (value in experiment) {
    tidyTail(experiment[count], platename[count])
    tidySource(paste("TidyDataStep1_",platename[count],".csv",sep=""), platename[count])
    vectorFilename <<- c(vectorFilename, paste("TidyDataStep2_",platename[count],".csv",sep=""))
    count=count+1
    message("working...")
  }
  
  #Load information using COPASutils
  message("Load Information...")
  count=1
  vectorRaw <<- vector(mode="character", length=0)
  #vectorRawAll <- vector(mode="character", length=0)
  #vectorRawAdult <- vector(mode="character", length=0)
  #vectorRawNonAdult <- vector(mode="character", length=0)
  for (value in vectorFilename) {
    message("working...")
    assign(paste("Exp",count,"All",sep=""), readPlate(value), envir = .GlobalEnv)
    message("working...")
    assign(paste("Exp",count,"Adult",sep=""), readPlate(value, tofmin=tofcutoff), envir = .GlobalEnv)
    message("working...")
    assign(paste("Exp",count,"NonAdult",sep=""), readPlate(value, tofmax=tofcutoff), envir = .GlobalEnv)
    vectorRaw <<- c(vectorRaw, paste("Exp",count,"All",sep=""),paste("Exp",count,"Adult",sep=""),paste("Exp",count,"NonAdult",sep=""))
    #vectorRawAll <- c(vectorRawAll, paste("Exp",count,"All",sep=""))
    #vectorRawAdult <- c(vectorRawAdult, paste("Exp",count,"Adult",sep=""))
    #vectorRawNonAdult <- c(vectorRawNonAdult, paste("Exp",count,"NonAdult",sep=""))
    count=count+1
  }
  
  #Summarize data using COPASutils
  message("Summarize Information...")
  #vectorSum <- vector(mode="character", length=0)
  count=1
  for (value in vectorRaw) {
    assign(paste("sum",value,sep=""), summarizePlate(eval(parse(text = value))), envir = .GlobalEnv)
    count=count+1
    message("working...")
  }
  
  #Summarize data + compound info
  count=1
  for (value in vectorRaw) {
    assign(paste("sumDR",value,sep=""), summarizePlate(eval(parse(text = value)), compounds=compounds, concentrations=concentrations), envir = .GlobalEnv)
    count=count+1
    message("working...")
  }
  
  message("Generate Diagnostic Plots...")
  
  #Diagnostic: Plot frequency polygon of TOF, to see what size the worms are.
  count=1
  #toPlot <- c("Exp1All","Exp2All")
  for (value in toPlotAll) {
    ggplot (eval(parse(text = value)), aes(TOF)) + 
      geom_freqpoly(binwidth = 10) + 
      scale_x_continuous(breaks = seq(0, 2000, 200), lim = c(0, 2000)) + 
      geom_vline(xintercept=tofcutoff, linetype="dotted", colour="blue") + 
      theme(plot.title = element_text(hjust = 0.5)) +
      ggtitle(paste(platename[count]," Frequency Polygon, P0 vs F1 cutoff = TOF ",tofcutoff,sep=""))
    ggsave(filename=paste("All FrequencyPolygonTOF_",platename[count],".pdf",sep=""))
    count=count+1
  }
  count=1
  #toPlot <- c("Exp1All","Exp2All")
  for (value in toPlotAll) {
    ggplot (eval(parse(text = value)), aes(TOF)) + 
      ...
      geom_vline(xintercept=tofcutoff, linetype="dotted", colour="blue") + 
      theme(plot.title = element_text(hjust = 0.5)) +
      ylim(0,500) + #to zoom in
      ggtitle(paste(platename[count]," Frequency Polygon Zoom, P0 vs F1 cutoff = TOF ",tofcutoff,sep=""))
    ggsave(filename=paste("All FrequencyPolygonTOFZoom_",platename[count],".pdf",sep=""))
    count=count+1
  }
  
  
  #Diagnostic: Plot the scatter distribution of TOF and EXT on plate:
  count=1
  #toPlot <- c("Exp1All","Exp2All")
  for (value in toPlotAll) {
    ggplot (eval(parse(text = value)), aes(TOF, EXT)) + 
      geom_point(aes(color = TOF), size = 1, alpha = 1/2) + 
      ...
      geom_vline(xintercept=tofcutoff, linetype="dotted", colour="blue") + 
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," Scatter Distribution,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("All TOFxEXT_",platename[count],".png",sep=""))
    count=count+1
  }
  
  #Diagnostic: Plot the number of P0 on the plate:
  count=1
  #toPlot <- c("sumExp1Adult","sumExp2Adult")
  for (value in toPlotSumAdult) {
    plotTrait(eval(parse(text = value)), "n") +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," P0 counted,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("P0 counted_",platename[count],".pdf",sep=""))
    count=count+1
  }
  
  #Diagnostic: Plot the number of F1 on the plate:
  count=1
  #toPlot <- c("sumExp1NonAdult","sumExp2NonAdult")
  for (value in toPlotSumNonAdult) {
    plotTrait(eval(parse(text = value)), "n") +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," F1 counted,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 counted_",platename[count],".pdf",sep=""))
    count=count+1
  }
  
  message("Generate Data Plots...")
  
  #Plot TOF per well
  count=1
  #toPlot <- c("Exp1NonAdult","Exp2NonAdult")
  for (value in toPlotNonAdult) {
    plotTrait(eval(parse(text = value)), "TOF", type="hist") +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," F1 TOF,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 TOF_",platename[count],".pdf",sep=""))
    count=count+1
  }
  
  #Plot TOF x EXT per well
  count=1
  #toPlot <- c("Exp1NonAdult","Exp2NonAdult")
  for (value in toPlotNonAdult) {
    plotTrait(eval(parse(text = value)), "TOF", "EXT", type="scatter") +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," F1 TOF x EXT,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 TOFxEXT_",platename[count],".png",sep=""))
    count=count+1
  }
  
  #Plot compare
  if(loops>1){
    plotCompare(list(Exp1NonAdult, Exp2NonAdult), "TOF", plateNames=c(platename[1], platename[2]))+
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[1], " vs ", platename[2]," F1 TOF,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 TOF_",platename[1],"vs",platename[2],".pdf",sep=""))  
  }
  
  
  #plotCompare(list(sumExp1NonAdult, sumExp2NonAdult), "mean.TOF", plateNames=c(platename1, platename2)) #-werktniet
  
  #Plot correlation matrix within plate
  count=1
  #toPlot <- c("sumExp1NonAdult","sumExp2NonAdult")
  for (value in toPlotSumNonAdult) {
    plotCorMatrix(eval(parse(text = value)),eval(parse(text = value))) +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count], " vs ", platename[count]," F1 CorMatrix,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 CorMatrix_",platename[count],"vs",platename[count],".pdf",sep=""))
    count=count+1
  }
  
  #Plot correlation matrix between plates
  if(loops>1){
    plotCorMatrix(sumExp1NonAdult,sumExp2NonAdult) +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[1], " vs ", platename[2]," F1 CorMatrix,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 CorMatrix_",platename[1],"vs",platename[2],".pdf",sep=""))  
  }
  
  #Plot mean.TOF per compound
  count=1
  #toPlot <- c("Exp1NonAdult","Exp2NonAdult")
  for (value in toPlotNonAdult) {
    toPlotSum <- summarizePlate((eval(parse(text = value))), compounds=compounds)
    plotDRTrait(toPlotSum, concentrations=concentrations, trait="mean.TOF") +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," F1 meanTOFxCompound,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep="")) +
      ylim(0,(max(toPlotSum$mean.TOF, na.rm=TRUE)))
    ggsave(filename=paste("F1 meanTOFxCompound_",platename[count],".pdf",sep=""), width = 210, height = 297, units = "mm")
    count=count+1
  }
  
  #plotCompound
  count=1
  #toPlotSum <- c("sumDRExp1NonAdult","sumDRExp2NonAdult")
  for (value in toPlotSumDRNonAdult) {
    ggplot (eval(parse(text = value)), aes(log10(concentration), n)) + 
      geom_point(aes(color = compound), size = 2, alpha = 1/2) +
      scale_colour_discrete(guide = FALSE) +
      facet_wrap(~compound, scales="free", ncol=2) + 
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
      stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1) + 
      ylim(-2000,(max(eval(parse(text = value))$n+1000, na.rm=TRUE))) +
      theme(plot.title = element_text(hjust = 0.5)) + 
      ggtitle(paste(platename[count]," F1 NxCompound,\n(P0 vs F1 cutoff = TOF ",tofcutoff,")",sep=""))
    ggsave(filename=paste("F1 NxCompound_",platename[count],".pdf",sep=""), width = 210, height = 297, units = "mm")
    count=count+1
  }
  
  message("\nI'm finished! :)\nAll the plots have been saved to file\n")
