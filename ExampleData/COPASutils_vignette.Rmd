...
We now see that we have many more trait variables, many of which describe the distribution of the originally measured values by well. We can get an even more complete picture by adding in extra quantiles, log transformed values of EXT and the fluorescent channels, and the minimum and maximum of each trait, as below:

```{r}
setupSummarized <- summarizePlate(setupRaw, quantiles=TRUE, log=TRUE, ends=TRUE)
```
